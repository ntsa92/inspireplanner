...
export default class GanttChart extends LightningElement {
  ...
  setStartDate(_startDate) {
    if (_startDate instanceof Date && !isNaN(_startDate)) {
      ...
      this.startDate = momentJS(_startDate)
        .day(1)
        .toDate();
      this.startDateUTC =
        momentJS(this.startDate)
          .utc()
          .valueOf() -
        momentJS(this.startDate).utcOffset() * 60 * 1000 +
        "";
  ...
  setDateHeaders() {
    this.endDate = momentJS(this.startDate)
      .add(this.view.slots * this.view.slotSize - 1, "days")
      .toDate();
    this.endDateUTC =
      momentJS(this.endDate)
        .utc()
        .valueOf() -
      momentJS(this.endDate).utcOffset() * 60 * 1000 +
      "";
    ...
    for (let date = momentJS(this.startDate); date <= momentJS(this.endDate); date.add(this.view.slotSize, "days")) {
      ...
      if (this.view.slotSize > 1) {
        let end = momentJS(date).add(this.view.slotSize - 1, "days");
        day.end = end.toDate();
      } else {
        day.end = date.toDate();
  ...
  handleRefresh() {
    ...
    getChartData({
        ...
        filterProjectRecords: self._filterData.projectRecordType, // filter for record types
